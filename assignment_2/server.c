...
int main(void) {
	...
	if(mkfifo(NP_SEND,0666) == -1){printf("mkfifo send return -1");return -1;}
	if(mkfifo(NP_RECEIVE,0666) == -1){printf("mkfofo receive return -1");return -1;}
	if((receive_fd = open(NP_RECEIVE,O_RDWR))==-1){printf("receiver open failed");return -1;}
